...
class Model:
    ...
    def forward_pass(self, inputs):
        ...
        # vector_list = np.zeros((len(inputs)))
        vector_list = np.dot(inputs, self.W) + self.b

        # for img_index, image in enumerate(inputs):
        #     output_vector = np.zeros((len(image)))
        #     for index, neuron in enumerate(image):
        #         logit = 0
        #         for i in range(self.num_classes):
        #             logit += self.W[index][i] * neuron + self.b[i]
        #         output_vector[index] = logit
        #     vector_list[img_index] = output_vector


        # TODO: Get probabilities by using softmax on the logits
        e_x = np.exp(vector_list - np.max(vector_list))
        softmax = e_x / np.sum(e_x)
        # for index, vector in enumerate(vector_list):
        #     e_x = np.exp(vector - np.max(vector))
        #     softmax = e_x / np.sum(e_x)
        #     vector_list[index] = softmax
    ...
    def loss(probabilities, gt_label):
        ...
        # loss = np.zeros((len(probabilities)))
        ...
        #         if prob == gt_label:
        #             yj = 1
        #         single_loss += yj * np.log(prob + 1E-10)
        #     loss[index] = - single_loss

        loss = -np.log(probabilities[0][gt_label]+1E-10)

        # While the batch size is only 1 in this assignment, we ask that you return probabilities of the shape (batch_size, num_classes), where batch_size will always be 1. You can hardcode in your loss function probabilities[0][gt_label] if you wish.
    ...
    def back_propagation(img, probabilities, gt_label):
        ...
        """ # loss = -np.log(prob[0][gt_label]+1E-10)
        ...
        gradB = probabilities
        gradB[0][gt_label] = gradB[0][gt_label] - 1

        gradW = np.dot(img, gradB) # note order

        # TODO: Reshape train image data to be matrix, dimension [784, 1]

        # "Reshape train image data to be matrix, dimension [784, 1]". While this works for the MNIST data, it will throw an error when using SceneRec. To fix this, you can reshape to (-1, 1), which makes NumPy infer the unspecified value to be the length of the image (784 in the case of MNIST and 4096 in SceneRec).
    ...
    def train_nn(self):
        ...
        connected with a softmax unit, and the loss function is cross-entropy
        loss.
        ...
        for epoch in range(self.num_epochs):
            ...
            print("Epoch Accuracy: ", self.accuracy_nn(self.train_images, self.train_labels))
